...
## ERC terrain & persistent rock perception upgrade (Parts 1-38)

This revision adds a slope-aware local terrain model, an explicit
rock/terrain/step/unknown classification stage, a persistent map-frame
rock landmark database, and explicit perception-health/failure
semantics, on top of the pipeline described above. See
`docs/erc_upgrade_report.md` in this package for the full audit,
acceptance-criteria checklist, and honest test/limitation report.

### New modules (pure computation, zero ROS dependency)

| Module | Purpose |
|---|---|
| `terrain_model.py` | Grid-based local plane fitting; height/slope/roughness relative to the LOCAL terrain surface, not one global threshold. |
| `terrain_classification.py` | TERRAIN / ROCK / OBSTACLE / STEP / UNKNOWN decision using several features together. |
| `confidence.py` | 0..1 confidence from point count, density, depth consistency, distance, terrain separation. |
| `perception_health.py` | VALID / DEGRADED / INVALID perception states; gates whether the occupancy grid may report FREE this frame. |
| `rock_landmarks.py` | Persistent `rock_N` identity, map-frame storage, re-identification, EMA position updates, visibility memory, optional JSON save/load. |

### Updated pipeline position

```
... (5) Ground removal -> ground_xyz, obstacle_xyz
        |
        v
[5.5] LocalTerrainModel.fit(ground_xyz)          (terrain_model.py)
        |
... (6-8) Voxel / outlier / DBSCAN / feature extraction (unchanged)
        |
        v
[8.5] classify_cluster() per cluster              (terrain_classification.py)
        |         TERRAIN -> dropped (Part 4 fix)
        |         UNKNOWN -> kept separately, rasterized as UNKNOWN cells
        |         ROCK/OBSTACLE/STEP -> continues below
        v
[9] Temporal obstacle tracking (unchanged, ROCK/OBSTACLE/STEP only)
        |
        v
[9.5] RockLandmarkDatabase.update()  (map-frame, persistent IDs)   (rock_landmarks.py)
        |
        v
[10] Occupancy grid (obstacles + uncertain regions), costmap (unchanged math)
```

### New topics

- `/terrain/perception_health` (`std_msgs/String`) -- `VALID` / `DEGRADED: ...` / `INVALID: ...`.
- `/terrain/rock_landmarks` (`std_msgs/String`, JSON) -- full persistent landmark database, structured (Part 32). See the report for why this is JSON-over-String rather than a new `.msg` type.
- `/terrain/rock_landmark_markers` (`visualization_msgs/MarkerArray`) -- map-frame landmark visualization, dimmed when `currently_visible=False`.

### New parameters

See `terrain_node.py`'s `_declare_parameters` for the full list and defaults: `terrain_patch_size`, `terrain_fit_min_points`, `terrain_fit_max_residual`, `max_traversable_slope_deg`, `caution_slope_deg`, `max_terrain_residual`, `min_rock_height`, `min_rock_width`, `min_rock_depth`, `min_rock_points`, `rock_association_distance`, `rock_position_gate` (reserved), `landmark_timeout_sec`, `confidence_threshold`, `map_frame`, `odom_frame`, `landmarks_save_path`, `max_cloud_age_sec`, `min_valid_points_health`, `min_ground_points_health`, `max_processing_latency_sec`, `enable_negative_obstacle_marking`, plus `enable_terrain_classification` / `enable_rock_landmarks` kill switches that restore the exact pre-upgrade behavior when set False.
