...
| **TF Lookup & Coordinate Caching** | ✅ **SOLVED** | `CloudFrameTransformer` in `tf_transform.py` caches the $4 \times 4$ homogeneous transformation matrix from `camera_depth_optical_frame` to `base_link`, preventing node crashes during boot latency. |
...
#### 🔴 The Problem
When the rover turns or maneuvers around an obstacle, the obstacle leaves the camera's active field of view ($69^\circ \times 42^\circ$ FOV) and instantly disappears from perception. As a result, Nav2's local planner loses awareness of obstacles in blind spots, risking collisions during sharp swerves.
...
2. Implement distance gating ($d \le 3.5\text{m}$) and temporal confidence gating ($\ge 3$ consecutive confirmed frames).
