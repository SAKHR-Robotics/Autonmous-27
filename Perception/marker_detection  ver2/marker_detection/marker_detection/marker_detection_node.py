...
from marker_detection_msgs.msg import (Marker3DDetection, Marker3DDetectionArray,
                                  MarkerDetection, MarkerDetectionArray, MarkerPose, MarkerPoseArray,
                                  MarkerTrackedPose, MarkerTrackedPoseArray)
from marker_detection import diagnostics as diag
from marker_detection import image_quality
...
from marker_detection.detection_validator import DetectionValidator, ValidatedMarker
from marker_detection.depth_processor import DepthEstimate, DepthProcessor
from marker_detection.image_processor import bgr_to_gray
from marker_detection.pose_estimator import PoseEstimate, PoseEstimator
from marker_detection.quality_flags import QualityFlag
from marker_detection.sync_buffer import DepthSyncBuffer
from marker_detection.tracker_manager import RawPose, TrackerManager, TrackedPose, TrackState, select_canonical_with_source
...
class MarkerDetectionNode(Node):
    ...
    def __init__(self) -> None:
        ...
        self._camera_info_stale_warned = False
        ...
        self._counts = dict(detections=0, rejected_quality=0, rejected_pose=0,
                            ...
                            frames_with_depth=0, frames_without_depth=0, duplicate_id_events=0)
...
<<<<<<< HEAD:Perception/marker_detection  ver2/marker_detection/marker_detection/marker_detection_node.py
        ...
        # aruco_dictionary pattern) but cv2.aruco.DetectorParameters.cornerRefinementMethod is
        # an int enum; resolve_corner_refinement_method does that translation and is itself
...
>>>>>>> 18eebae ( solvint proplems with the marker detection and the terrian geo):Perception/marker_detection/marker_detection/marker_detection_node.py
    ...
    def _log_startup_self_check(self) -> None:
        """Section 32: a single, unmissable summary of the runtime configuration
        that actually took effect, so a topic/dictionary/marker-size mismatch is
        visible immediately in the log rather than discovered from bad poses
        later. CameraInfo/TF readiness cannot be known synchronously at __init__
        time (no messages have arrived yet); those are covered by the periodic
        diagnostics (_publish_diagnostics) and the _report_sync_health warnings
        once the node is spinning."""
        mode = "COMPETITION" if self._competition_mode else "DEVELOPMENT"
        p = self.get_parameter
        lines = [
            "===== marker_detection startup self-check =====",
            f"MODE: {mode}",
            f"RGB_TOPIC: {p('rgb_topic').value}",
            f"DEPTH_TOPIC: {p('depth_topic').value}",
            f"CAMERA_INFO_TOPIC: {p('camera_info_topic').value}",
            f"ARUCO_API: {self.detector.api_name}",
            f"DICTIONARY: {p('aruco_dictionary').value}",
            f"MARKER_SIZE: {self._default_marker_size_m:.3f} m"
            + (" (per-marker overrides ENABLED)" if self._marker_sizes else ""),
            f"CORNER_REFINEMENT: {p('cornerRefinementMethod').value}",
            f"DEPTH_SCALE: {p('depth_scale').value} (16UC1 only; 32FC1 is used as-is)",
            f"DEPTH_RANGE: [{p('min_depth_m').value:.2f}, {p('max_depth_m').value:.2f}] m",
            f"SIM_TIME: {bool(self.get_parameter('use_sim_time').value if self.has_parameter('use_sim_time') else False)}",
            "CAMERA_INFO: PENDING (no message received yet)",
            "=================================================",
        ]
        for line in lines:
    ...
    def _declare_parameters(self) -> None:
        defaults: dict[str, Any] = {
            ...
            # Priority 3 (engineering audit follow-up): CameraInfo's age was already reported
            # diagnostically but never enforced -- intrinsics received once could silently be
            # reused forever, including after the RealSense driver dies. Once CameraInfo is
            # older than this, pose estimation is skipped for that frame (2D detection is
            # unaffected) until a fresh CameraInfo arrives. 5 s matches this node's existing
            # RGB/depth staleness convention (`sync_stale_warn_s`/`_report_sync_health`).
            "camera_info_max_age_s": 5.0,
            "detection_topic": "/marker_detections",
            # Verified against the physical/simulation markers in aruco.zip: all 15 markers
            # decode uniquely under DICT_4X4_50 (IDs 1-15). Must match config/marker_detection.yaml,
            # scripts/generate_aruco_marker.py, and the simulation models -- single source of truth.
            ...
            "debug_3d_image_topic": "/marker_detection/debug_3d_image",
            ...
            "raw_pose_topic": "/marker_poses_raw", "marker_pose_topic": "/marker_poses",
            # Side length in metres of the printed marker's outer black border. The aruco.zip
            # cube models place a 0.21 x 0.21 m marker face on each head (model.sdf); this is
            # the authoritative physical size for the current competition marker set -- must
            # match config/marker_detection.yaml's marker_size_m.
            "marker_size_m": 0.21,
            ...
            "per_marker_size_ids": [0], "per_marker_size_values": [0.21],
            ...
            # Section 7: ArUco corner refinement. Declared as a readable string name
            # (translated to the cv2.aruco.CORNER_REFINE_* int constant in __init__,
            # same pattern as aruco_dictionary) so the parameter is self-documenting.
            # Sub-pixel refinement materially reduces corner jitter and therefore
            # PnP/distance jitter; it is the default unless testing proves otherwise.
            "cornerRefinementMethod": "CORNER_REFINE_SUBPIX",
            "cornerRefinementWinSize": 5, "cornerRefinementMaxIterations": 30,
        ...
        for name, value in defaults.items():
            if name == "allowed_marker_ids":
                self.declare_parameter(
                    name,
                    value,
                    ParameterDescriptor(type=ParameterType.PARAMETER_INTEGER_ARRAY, dynamic_typing=True)
                )
            elif name == "per_marker_size_ids":
                self.declare_parameter(
                    name,
                    value,
                    ParameterDescriptor(type=ParameterType.PARAMETER_INTEGER_ARRAY, dynamic_typing=True)
                )
            elif name == "per_marker_size_values":
                self.declare_parameter(
                    name,
                    value,
                    ParameterDescriptor(type=ParameterType.PARAMETER_DOUBLE_ARRAY, dynamic_typing=True)
                )
            else:
    ...
    def _validate_parameters(self) -> None:
        ...
        if p("camera_info_max_age_s").value <= 0:
            raise ValueError("camera_info_max_age_s must be positive.")
    ...
    def _build_marker_size_overrides(self) -> dict[int, float]:
        if not self.get_parameter("use_per_marker_sizes").value:
            return {}
        ids = self.get_parameter("per_marker_size_ids").value or []
        values = self.get_parameter("per_marker_size_values").value or []
    ...
    def _camera_info_is_stale(self) -> bool:
        """Priority 3: thin wrapper around the pure, independently-tested
        `diag.camera_info_is_stale` (test_diagnostics.py) -- kept here only to
        own the one-shot warning log and the ROS parameter lookup."""
        age_s = (time.monotonic() - self._camera_info_received_monotonic
                 if self.camera_info is not None and self._camera_info_received_monotonic is not None else None)
        max_age = float(self.get_parameter("camera_info_max_age_s").value)
        stale = diag.camera_info_is_stale(age_s, max_age)
        if stale and not self._camera_info_stale_warned:
            self.get_logger().warning(
                f"CameraInfo is older than camera_info_max_age_s={max_age:.1f}s "
                "(publisher stalled or disconnected?); pose estimation is skipped "
                "until a fresh CameraInfo arrives. 2D detection is unaffected.")
            self._camera_info_stale_warned = True
        return stale
    ...
    def _rgb_callback(self, rgb_message: Image) -> None:
        ...
        tracked, canonical_poses = self._track_poses(poses, rgb_message)
        ...
        if not self._competition_mode:
            if self.get_parameter("publish_debug_image").value:
                self._publish_debug(rgb_message, bgr, canonical_poses, elapsed)
    ...
    def _estimate_poses(self, associated: list[AssociatedMarker], timestamp: float) -> list[AssociatedPose]:
        ...
        # Priority 3: computed once per frame, not per marker -- a stale check is a property
        # of the CameraInfo message, not of any individual detection.
        camera_info_stale = self._camera_info_is_stale()
        effective_camera_info = None if camera_info_stale else self.camera_info
        for item in associated:
            ...
            pose = estimator.estimate(item.marker.corners, effective_camera_info, depth_position=depth_position,
...
>>>>>>> 18eebae ( solvint proplems with the marker detection and the terrian geo):Perception/marker_detection/marker_detection/marker_detection_node.py
    ...
    def _track_poses(self, poses: list[AssociatedPose], image: Image
                     ) -> tuple[list[TrackedPose], list[AssociatedPose]]:
        raw = []
        for item in poses:
        ...
        timestamp = float(stamp.sec) + float(stamp.nanosec) * 1e-9
        # Priority 1: `select_canonical_with_source` reuses TrackerManager.select_canonical
        # (the exact same deterministic same-ID selection `process()` uses below) purely to
        # recover *which AssociatedPose* (with its 2D corners, needed for drawing) each
        # canonical RawPose came from. Same pure function, not a second implementation.
        _canonical_raw, canonical_poses, _duplicate_ids = select_canonical_with_source(raw, poses)
        tracked = self.tracker_manager.process(raw, timestamp)
        self._counts["duplicate_id_events"] += sum(
            1 for item in tracked if QualityFlag.MULTIPLE_INSTANCES_SAME_ID & item.quality_flags)
        return tracked, canonical_poses
    ...
    def _camera_frame_z_m(self, item: AssociatedPose) -> float | None:
        """Priority 2 Requirement A: the camera-frame Z already available in the
        detection pipeline, for the primary debug image. This is deliberately
        NOT `distance_to_marker` (the base_link Euclidean distance published on
        `/marker_detection/targets`, computed downstream via TF2 in
        marker_action_interface_node.py / action_target_builder.py) -- the two
        are different quantities and must not be conflated. Prefers the PnP
        solution's Z (more precise at typical marker ranges) and falls back to
        the depth-sensor Z only when PnP did not produce a valid pose."""
        if item.pose.valid and item.pose.tvec is not None:
            return float(item.pose.tvec.reshape(3)[2])
        if item.association.position is not None:
            return float(item.association.position[2])
        return None

    def _publish_debug(self, image: Image, bgr: np.ndarray, poses: list[AssociatedPose], elapsed: float) -> None:
        """Priority 1: draws the canonical (same-ID-deduplicated) observations
        selected by `TrackerManager.select_canonical` -- the exact same
        selection tracking uses -- not the raw, pre-deduplication marker list.
        If two physical faces shared an ID this frame, only the one the
        pipeline actually selected is drawn here, never both.
        """
        debug = bgr.copy()
        for item in poses:
            marker = item.association.marker
            ...
            # Priority 2, Requirement A: camera-frame Z, explicitly labeled "Z" (not
            # "distance") so it is never confused with distance_to_marker (base_link,
            # Euclidean, published on /marker_detection/targets -- see _camera_frame_z_m).
            z_m = self._camera_frame_z_m(item)
            z_label = f"Z: {z_m:.2f} m" if z_m is not None else "Z: N/A"
            cv2.putText(debug, z_label, (center[0] + 6, center[1] + 16),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
        if self.get_parameter("show_debug_info").value:
            cv2.putText(debug, f"markers: {len(poses)}  {elapsed * 1000:.1f} ms", (10, 25),
    ...
    def _publish_diagnostics(self) -> None:
        ...
        status.values = [
            ...
            KeyValue(key="camera_info_stale", value=str(self._camera_info_is_stale())),
            KeyValue(key="camera_info_max_age_s", value=f"{float(self.get_parameter('camera_info_max_age_s').value):.2f}"),
            ...
            KeyValue(key="duplicate_id_events", value=str(self._counts["duplicate_id_events"])),
        ]
