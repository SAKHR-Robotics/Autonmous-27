...
def resolve_corner_refinement_method(name: str) -> int:
    """Translate a readable cornerRefinementMethod name (e.g. 'CORNER_REFINE_SUBPIX')
    to the cv2.aruco int constant DetectorParameters.cornerRefinementMethod expects.

    Section 7: corner refinement parameters were previously declared nowhere, so
    OpenCV always ran with its own default (CORNER_REFINE_NONE) regardless of any
    intent to configure it. Mirrors the aruco_dictionary string-name pattern so the
    ROS parameter stays self-documenting instead of an opaque integer 0-3.
    """
    if not name.startswith("CORNER_REFINE_") or not hasattr(cv2.aruco, name):
        raise ValueError(
            f"Unknown cornerRefinementMethod '{name}'; expected one of "
            f"CORNER_REFINE_NONE, CORNER_REFINE_SUBPIX, CORNER_REFINE_CONTOUR, CORNER_REFINE_APRILTAG.")
