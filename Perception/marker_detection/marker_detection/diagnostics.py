...
def camera_info_is_stale(age_s: float | None, max_age_s: float) -> bool:
    """Priority 3 (engineering audit follow-up): True once a previously-received
    CameraInfo is older than `max_age_s` and should no longer be trusted for PnP.

    `age_s=None` means no CameraInfo has ever been received -- a distinct
    failure mode from "stale" (a message that arrived once and was never
    refreshed again, e.g. a disconnected driver). It is never "stale" here;
    the caller already gets correct behavior for a missing CameraInfo for
    free, since passing `camera_info=None` into PoseEstimator.estimate()
    already yields an invalid pose independent of this check.
    """
    if age_s is None:
        return False
