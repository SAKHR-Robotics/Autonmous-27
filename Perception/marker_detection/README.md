...
7. **Explicit quality flags** (`quality_flags.py`), a `uint32` bitmask on
   ...
   TF_UNAVAILABLE`. A mission layer can act on the specific reason rather than
   inferring it from a single scalar.
...
rqt_image_view /marker_detection/3d_debug_image
...
- Cross-checks (via `ast`/`yaml` parsing, not just eyeballing) that every
  ...
  case.
- **85 of 85** runnable unit tests (up from 54 before this pass: +31 new tests
  across `test_sync_buffer.py`, `test_frame_transform.py`,
  `test_action_target_builder.py`, `test_action_interface_integration.py`, and
  additions to the existing `test_tracking.py`/`test_diagnostics.py` for
  `age_seconds`/sync-delta-stats; 91 total test functions in `test/`, of
  which 6 in `test_marker_tf_broadcaster.py` need real `rclpy`/`tf2_ros` and
  could not run here), using a minimal local stub for
  ...
  to additionally execute `test_camera_model.py` and `test_pose_estimator.py`:
  **all pass**.
...
- **Image** on `/marker_detection/debug_image`, `/marker_detection/3d_debug_image`,
